...
erpnext.taxes_and_totals = class TaxesAndTotals extends erpnext.payments {
	...
	apply_pricing_rule_on_item(item) {
		...
		// Net rate was typed manually while keeping discount %; do not re-derive % from discount_amount
		if (item._manual_rate_fixed_discount_pct && flt(item.discount_percentage) > 0) {
			let manual_rate = flt(item.rate, precision("rate", item));
			item.discount_amount = flt(item.rate_with_margin) - manual_rate;
			frappe.model.set_value(item.doctype, item.name, "discount_amount", item.discount_amount);
			frappe.model.set_value(item.doctype, item.name, "rate", manual_rate);
			return;
		}

		item_rate = flt(item.rate_with_margin, precision("rate", item));

		// If discount is %-led, always derive amount and net rate from % (do not overwrite % from stale discount_amount)
